...
export default function Home() {
  ...
  const addUser = () => {
    ...
    const userLivro = document.getElementById("userLivro") as HTMLInputElement

    const newUser = {
      nome: userNome.value,
      idade: userIdade.value,
      time: userLivro.value
    }

    if(userNome.value == "" || userIdade.value == "" || userLivro.value == ""){
      alert("Por favor preencha todos os campos.")
    }else{
      api.post("http://localhost:3000/users", newUser).then((response) => {
  ...
  const updatedUser = async  (id:any) => {
    ...
    const updatedLivro = document.getElementById (`updatedLivro${id}`) as HTMLInputElement;

    const updatedUser = {
      nome: updatedNome.value,
      idade: updatedIdade.value,
      time: updatedLivro.value
    };
      ...
      <table className="w-full table-auto border-collapse">
        <thead>
          <tr>
            ...
            <th className="border border-gray-500 px-4 py-2">Livro Favorito</th>
          </tr>
        </thead>
        <tbody>
          {user.map((user: any) => {
